<#
  ...
  Optional. Give the desired tag that should be used for favorites.
  Defaults to 'favorite' if not given.

  .OUTPUTS
  Console logs, and a failed batch file for retry.
...
$itemsToTag = $allItems | Where-Object { $null -eq $_.tags -or $FavoriteTag -notin $_.tags }
...
Write-Host "Tag Added:                        '$FavoriteTag'"
